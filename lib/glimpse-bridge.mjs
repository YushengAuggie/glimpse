...
function runAction(m){
  ...
  const kind=(m.action==="share")?"share":"export";
  const slug=String(m.slug||"");
  if(!slug) return {clientId,ok:false,kind,error:"missing slug"};
  try{
    if(kind==="export"){
      const out=execFileSync("bash",[BIN,"export",slug],{encoding:"utf8"});
      return {clientId,ok:true,kind,path:parseExportPath(out)};
    }
    // Share is PRIVATE by default (a generated password), matching the CLI — the
    // canvas confirm restates the egress before we ever get here. opts is reserved.
    const out=execFileSync("bash",[BIN,"share",slug],{encoding:"utf8"});
    const r=parseShareResult(out);
    return {clientId,ok:true,kind,url:r.url,password:r.password};
  }catch(e){
    const msg=String((e&&e.stderr)||(e&&e.message)||e).trim().split("\n").filter(Boolean).pop()||"failed";
    return {clientId,ok:false,kind,error:msg.slice(0,300)};
  }
}
