...
function runAction(m){
  ...
  try{
    ...
    if(action==="share"){
      ...
      const opts=m.opts||{};
      // A password-changing UPDATE applies at ht-ml.app's edge asynchronously — the
      // new password can take up to ~a minute to activate and the previous one may
      // linger briefly. Surface that so a "wrong password" test right after Update
      // reads as expected lag, not a failure. (New shares apply immediately.)
      const note=(opts.update && !opts.public && r.password)
        ? "New password may take up to ~a minute to activate; the previous one can work briefly."
        : "";
      return {clientId,ok:true,kind:"share",slug,url:r.url,password:r.password,
              visibility:r.password?"private":"public",update:!!opts.update,...(note?{note}:{})};
    }
    // Export: write a standalone offline copy to disk. A UI/daemon-initiated export
    // inherits the DAEMON's cwd (typically "/"), so `glimpse export` with no --out
    // would default to $PWD/<slug>.export.html → "//<slug>…" on a read-only root.
    // Pin it to a known, always-writable, discoverable dir ($GLIMPSE_DIR/exports)
    // so the file is predictable and retrievable; the toast shows the real path.
    let outPath="";
    try{ const exdir=join(DIR,"exports"); mkdirSync(exdir,{recursive:true}); outPath=join(exdir,`${slug}.export.html`); }catch{ outPath=""; }
    const exportArgs=outPath?["export",slug,"--out",outPath]:["export",slug];
    const out=execFileSync("bash",[BIN,...exportArgs],{encoding:"utf8"});
