...
// Target the page to snapshot the same way read/shot/interact do: with a URL, the
// app's own tab (a #slug maps to the canvas-host artifact file, so it lands on the
// canvas tab, not the app); with none, the current non-canvas app tab.
const { send, waitEvent, close, created, closeTab } = await cdpConnectApp(url);
...
// Close the throwaway tab we opened; a reused app/canvas tab just drops the socket.
if (created) { await closeTab(); } else { close(); }
