...
async function cdpConnect(pick){
  ...
  // `created` is true only when no existing target matched and we had to OPEN a
  // fresh tab. One-shot verbs (read/shot/snapshot) use it to close the throwaway
  // tab they created — otherwise every review of a not-yet-open URL leaks a tab
  // into the shared Chrome forever (there is no other tab GC). Reused tabs (from
  // `glimpse open`) stay `created:false` and are left alone.
  let created=false;
  if(!p){ created=true;
    p = await fetch(`${base}/json/new?about:blank`,{method:"PUT"}).then(r=>r.json())
  ...
  // close() drops only the WebSocket (the tab stays open — correct for canvas /
  // reused app tabs). closeTab() additionally closes the browser TAB via the same
  // DevTools HTTP endpoint family as /json/new, so a verb that created a throwaway
  // tab can clean it up. Best-effort: never throw out of teardown.
  const closeTab=async()=>{ try{ if(p&&p.id) await fetch(`${base}/json/close/${p.id}`); }catch(_){}
    try{ ws.close(); }catch(_){} };
  return { target:p, created, send, waitEvent, on, close:()=>ws.close(), closeTab };
}
